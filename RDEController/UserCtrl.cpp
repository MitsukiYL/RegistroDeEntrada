...
List<user^>^ UserCtrl::buscarUserAll() {
	...
	for each (String ^ lineaUser in lineas) {
		...
		bool permission = Convert::ToBoolean(datos[11]);
		String^ occupation = datos[12];
		String^ gender = datos[13];
		int age = Convert::ToInt32(datos[14]);
		bool isInside = Convert::ToBoolean(datos[15]);
...
void UserCtrl::escribirArchivo(List<user^>^ listaUser) {
	...
	for (int i = 0; i < listaUser->Count; i++) {
		...
		lineasArchivo[i] = Convert::ToString(objUser->getUserID()) + ";" + objUser->getUserType() + ";" + Convert::ToString(objUser->getActive()) 
			...
			+ ";" + Convert::ToString(objUser->getDNI()) + ";" + objUser->getName() + ";" + Convert::ToString(objUser->getCode())
			+ ";" + objUser->getMail() + ";" + objUser->getPhone() + ";" + objUser->getPassword() + ";" + Convert::ToString(objUser->getPermission())
			+ ";" + objUser->getOccupation() + ";" + objUser->getGender() + ";" + Convert::ToString(objUser->getAge()) + ";" + Convert::ToString(objUser->getIsInside());
	}
...
void UserCtrl::actualizarUser(int userID, String^ userType, bool active, int registrationDate, parkingSite^ objParkingSite, int DNI, String^ name, int code, String^ mail, String^ phone, String^ password, bool permission, String^ occupation, String^ gender, int age, bool isInside){
	...
	for (int i = 0; i < listaUser->Count; i++) {
		if (listaUser[i]->getUserID() == userID) {
			...
			listaUser[i]->setDNI(DNI);
			listaUser[i]->setName(name);
			listaUser[i]->setCode(code);
			listaUser[i]->setMail(mail);
			listaUser[i]->setPhone(phone);
			listaUser[i]->setPassword(password);
			listaUser[i]->setPermission(permission);
			listaUser[i]->setOccupation(occupation);
			listaUser[i]->setGender(gender);
			listaUser[i]->setAge(age);
			listaUser[i]->setIsInside(isInside);
